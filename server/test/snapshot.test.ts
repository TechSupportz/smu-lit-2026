...
describe('final snapshot invariants', () => {
  it('requires explicit case review and resolved material fact review', async () => {
    ...
    try {
      ...
      await expect(service.create(created.id, created.revision)).rejects.toThrow(ProcessingError);
      ...
      await expect(service.create(created.id, reviewedCase.revision)).rejects.toThrow(/material fact/);
  ...
  it('writes immutable, hashed JSON and links subsequent snapshots by supersession', async () => {
    ...
    try {
      ...
      expect(JSON.parse(firstText)).toMatchObject({
        schemaVersion: '1.0.0',
        snapshot: { id: first.record.id, caseId: created.id, caseRevision: reviewed.revision },
      });
      ...
      expect(store.getSnapshot(created.id, first.record.id).supersededBySnapshotId).toBe(second.record.id);
  ...
  it('preserves eligibility failures in the snapshot and marks handoff blocked', async () => {
    ...
    try {
      ...
      expect(result.snapshot.proceduralChecks.some((check) => check.code === 'AMOUNT_LIMIT' && check.result === 'FAIL')).toBe(true);
    } finally {
      store.close();
    }
  });
