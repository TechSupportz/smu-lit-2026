...
describe('CaseStore persistence and mutation safety', () => {
  ...
  it('rejects stale revisions instead of silently overwriting newer data', () => {
    ...
    try {
      ...
      expect(() => store.updateCase(created.id, {
        expectedRevision: created.revision,
        patch: { title: 'Stale writer' },
      })).toThrow(RevisionConflictError);
      expect(store.getCase(created.id)).toMatchObject({ revision: next.revision, title: 'First writer' });
    } finally {
      store.close();
    }
  });
  ...
  it('replays an idempotent create and rejects a reused key with a different request', () => {
    ...
    try {
      ...
      expect(() => createCase(store, { idempotencyKey: 'create-key-1', title: 'Different case' }))
        .toThrow(IdempotencyConflictError);

      const turn = store.beginTurn(first.id, { idempotencyKey: 'turn-key-1', requestHash: 'hash-a' });
      expect(turn.duplicate).toBe(false);
      store.finishTurnAdmission(turn.id, {
        submissionId: 'submission-1', uid: 'agent-1', acceptedAt: '2026-09-05T00:00:00.000Z',
      });
      const turnReplay = store.beginTurn(first.id, { idempotencyKey: 'turn-key-1', requestHash: 'hash-a' });
      expect(turnReplay).toMatchObject({ duplicate: true, status: 'ACCEPTED', submissionId: 'submission-1', agentUid: 'agent-1' });
      expect(() => store.beginTurn(first.id, { idempotencyKey: 'turn-key-1', requestHash: 'hash-b' }))
        .toThrow(IdempotencyConflictError);
    } finally {
      store.close();
    }
  ...
  it('acknowledges the exact warning fingerprint and makes a changed warning stale', () => {
    ...
    try {
      ...
      const first = refreshWithWarning(store, record.id, firstFingerprint, 'Review the amount');
      const warning = store.getCaseState(record.id).warnings.find((item) => item.code === 'TEST_WARNING');
      ...
      refreshWithWarning(store, record.id, 'fingerprint-v2', 'Review the changed amount', changedRevision);
      const activeWarnings = store.getCaseState(record.id).warnings;
      expect(activeWarnings).toHaveLength(1);
      expect(activeWarnings[0]).toMatchObject({ fingerprint: 'fingerprint-v2', status: 'OPEN' });
      expect(() => store.acknowledgeWarning(record.id, warning!.id, {
        expectedRevision: store.getCase(record.id).revision,
        fingerprint: firstFingerprint,
      })).toThrow(RevisionConflictError);
...
function refreshWithWarning(store: Parameters<typeof refreshAssessment>[0], caseId: string, fingerprint: string, message: string, expectedRevision?: number) {
