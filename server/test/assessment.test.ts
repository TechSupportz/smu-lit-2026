...
import { assessCase, refreshAssessment } from "../src/services/assessment.js"
...
describe("SCT eligibility assessment", () => {
    it.each([
        ...
        "applies the exact claim amount boundary at %d cents with consent %s",
        (claimAmountCents, consentStatus, expected) => {
            ...
            try {
                ...
                expect(result.checks.find(check => check.code === "AMOUNT_LIMIT")?.result).toBe(
                    expected,
                )
            } finally {
                store.close()
    ...
    it("keeps an unknown date unverified and does not turn it into a failure", () => {
        ...
        try {
            ...
            expect(result.checks.find(check => check.code === "TIME_LIMIT")?.result).toBe(
                "UNVERIFIED",
            )
    ...
    it("rejects an employment intake represented by the generic category subtype", () => {
        ...
        try {
            ...
            expect(result.checks.find(check => check.code === "DISPUTE_CATEGORY")).toMatchObject({
                result: "FAIL",
            })
        } finally {
            store.close()
    ...
    it("does not block the frontend CPFTA category on details collected after eligibility", () => {
        const { store } = makeStoreSync("assessment-cpfta-frontend")
        try {
            const record = createCase(store)
            const state = store.getCaseState(record.id)
            const result = assessCase(
                {
                    ...
                    case: {
                        ...
                        modelData: {
                            ...
                            consumerSupplierRelationship: null,
                            relationshipEstablished: null,
                            underlyingTransaction: null,
                            exactConduct: null,
                            occurredAt: null,
                            allegedLossCents: null,
                            remedyNotes: null,
                            evidenceIds: [],
                        },
                        causeOfActionDate: "2025-01-01",
            ...
            expect(result.checks.find(check => check.code === "CPFTA_CONDITIONS")).toMatchObject({
    ...
    it("removes category-specific checks after the category changes", () => {
        const { store } = makeStoreSync("assessment-category-change")
        try {
            const record = createCase(store)
            store.updateCase(record.id, {
            ...
            refreshAssessment(store, record.id, new Date("2026-01-01T00:00:00.000Z"))
            expect(
                store
                    .getCaseState(record.id)
                    .eligibilityChecks.find(check => check.code === "PROPERTY_DAMAGE_EXCLUSION"),
            ).toMatchObject({ result: "FAIL" })

            const current = store.getCase(record.id)
            store.updateCase(record.id, {
            ...
            expect(
                store
                    .getCaseState(record.id)
                    .eligibilityChecks.some(check => check.code === "PROPERTY_DAMAGE_EXCLUSION"),
            ).toBe(false)
        } finally {
            store.close()
        }
                ...
                assessCase(
                    {
                        ...
                        case: {
                            ...base.case,
                            causeOfActionDate: "2024-01-01",
                            causeOfActionDatePrecision: "EXACT",
                        },
                    },
                    new Date("2026-01-01T00:00:00Z"),
                ).checks.find(check => check.code === "TIME_LIMIT")?.result,
            ).toBe("PASS")
            expect(
                assessCase(
                    {
    ...
    it("treats a confirmed user fact as review-complete without calling it evidence-supported", () => {
        ...
        try {
            ...
            expect(
                result.warnings.some(warning => warning.code === `FACT_${fact.id}_UNREVIEWED`),
            ).toBe(false)
            expect(
                result.warnings.some(warning => warning.code === `FACT_${fact.id}_EVIDENCE`),
            ).toBe(true)
        } finally {
            store.close()
        }
