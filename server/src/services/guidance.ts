...
function excerpts(text: string, query: string): string[] {
  ...
  for (const term of terms) {
    ...
    while (hits.length < 8) {
      ...
      if (!hits.some((existing) => existing.includes(item) || item.includes(existing))) hits.push(item);
...
async function fetchAllowed(urlValue: string, timeoutMs: number): Promise<{ url: string; text: string }> {
  ...
  for (let redirect = 0; redirect <= 3; redirect += 1) {
    ...
    if (!contentType.includes('text/html') && !contentType.includes('text/plain')) {
      throw new Error(`Official source returned unsupported content type ${contentType || 'unknown'}`);
    }
    const body = await response.text();
    if (body.length > 5_000_000) throw new Error('Official source exceeded the response size bound');
...
export class GuidanceService {
  ...
  async search(query: string, sourceNames: Array<keyof typeof SOURCES> = ['eligibility', 'filing', 'statutes']): Promise<GuidanceResult> {
    const uniqueSources = [...new Set(sourceNames)];
    const results: GuidanceResult['sources'] = [];
    for (const sourceName of uniqueSources) {
      const source = SOURCES[sourceName];
      try {
        const response = await fetchAllowed(source, Math.min(this.appConfig.providerTimeoutMs, 20_000));
        results.push({ url: response.url, status: 'RETRIEVED', excerpts: excerpts(response.text, query) });
      } catch (error) {
        results.push({
          url: source,
          status: 'FAILED',
