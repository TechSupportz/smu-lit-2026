...
export class PdfService {
  ...
  async compile(caseId: string, snapshotId: string): Promise<PdfCompilationResult> {
    return this.store.mutations.run(caseId, async () => {
      ...
      if (!inside(this.projectRoot, snapshot.jsonPath) || !inside(this.projectRoot, this.templatePath)) {
        throw new ProcessingError('Typst compilation is restricted to the repository artifact workspace.');
      }
      const jsonBytes = await readFile(snapshot.jsonPath);
      if (sha256(jsonBytes) !== snapshot.jsonSha256) {
        throw new ProcessingError('Snapshot JSON failed its integrity check; PDF compilation was refused.');
      }

      const outputDirectory = dirname(snapshot.jsonPath);
      const finalPath = resolve(outputDirectory, `${snapshot.basename}.pdf`);
      const temporaryPath = resolve(outputDirectory, `.${snapshot.basename}.${nanoid(8)}.tmp.pdf`);
      const typstSnapshotPath = relative(dirname(this.templatePath), snapshot.jsonPath);
      try {
        await execFileAsync(this.appConfig.typstBin, [
          'compile',
          '--root', this.projectRoot,
          '--input', `snapshot=${typstSnapshotPath}`,
          this.templatePath,
          temporaryPath,
        ], {
        ...
        const updated = this.store.attachSnapshotPdf(caseId, snapshotId, finalPath, sha256(pdfBytes));
        return { snapshot: updated, pageCount, snapshotHashMatched: true, pdfHeaderValid: true };
      } catch (error) {
        await unlink(temporaryPath).catch(() => undefined);
        if (error instanceof ProcessingError) throw error;
        throw new ProcessingError('Typst compilation failed; the immutable JSON snapshot was preserved.', {
  ...
  private async pageCount(path: string): Promise<number> {
    ...
    if (!match) throw new ProcessingError('pdfinfo could not verify the compiled PDF page count.');
