...
export class FlueCleanupService {
    ...
    async purgeCase(
        caseId: string,
        timeoutMs = 10_000,
    ): Promise<{ submissions: number; streamRemoved: boolean }> {
        ...
        try {
            ...
            while (true) {
                const rows = database
                    .prepare("SELECT status FROM flue_agent_submissions WHERE session_key = ?")
                    .all(key) as Array<{ status: string }>
                if (!rows.some(row => ACTIVE_STATUSES.has(row.status))) break
                if (Date.now() >= deadline) {
                    throw new ProcessingError(
                        "Timed out waiting for active agent work to stop; case deletion was not started.",
                        {
                            caseId,
                            retryable: true,
                        },
            ...
            const streamRemoved =
                this.hasTable(database, "flue_conversation_streams") &&
                database
                    .prepare("SELECT 1 FROM flue_conversation_streams WHERE path = ?")
                    .get(path) !== undefined
            ...
            try {
                if (submissionIds.length > 0 && this.hasTable(database, "flue_submission_chunks")) {
                    ...
                    database
                        .prepare(
                            `DELETE FROM flue_submission_chunks WHERE submission_id IN (${placeholders})`,
                        )
                ...
                database
                    .prepare("DELETE FROM flue_agent_submissions WHERE session_key = ?")
                    .run(key)
