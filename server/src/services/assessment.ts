...
export const ELIGIBILITY_SOURCE_URL = 'https://www.judiciary.gov.sg/civil/cases-eligible-small-claim';
...
export interface ReadinessReason {
  code: string;
  message: string;
  kind: 'ELIGIBILITY' | 'MISSING_INFORMATION' | 'EVIDENCE' | 'CONTRADICTION' | 'REVIEW' | 'PRIVACY';
...
function fingerprint(code: string, inputs: unknown): string {
  return createHash('sha256').update(`${code}:${JSON.stringify(inputs)}`).digest('hex');
}
...
function amountCheck(record: CaseRecord): EligibilityCheckInput {
  ...
  if (record.consentStatus === 'YES') {
    return check('AMOUNT_LIMIT', 'PASS', 'The amount is above SGD 20,000 and not above SGD 30,000, with consent recorded.', {
      claimAmountCents: amount,
      consentStatus: record.consentStatus,
    });
  }
  if (record.consentStatus === 'NO') {
    return check('AMOUNT_LIMIT', 'FAIL', 'The amount exceeds SGD 20,000 and consent from both parties is confirmed absent.', {
      claimAmountCents: amount,
      consentStatus: record.consentStatus,
    });
  }
  return check('AMOUNT_LIMIT', 'UNVERIFIED', 'The amount is above SGD 20,000 and consent from both parties is not yet known.', {
    claimAmountCents: amount,
    consentStatus: record.consentStatus,
  });
}
...
function timeCheck(record: CaseRecord, today: Date): EligibilityCheckInput {
  ...
  if (!causeDate || record.causeOfActionDatePrecision !== 'EXACT') {
    return check('TIME_LIMIT', 'UNVERIFIED', 'An exact cause-of-action date is not available, so the 2-year filing period cannot be checked.', {
  ...
  if (causeDate > todayUtc) {
    return check('TIME_LIMIT', 'UNVERIFIED', 'The recorded cause-of-action date is in the future and needs review.', {
      causeOfActionDate: record.causeOfActionDate,
  ...
  return check('TIME_LIMIT', result,
    result === 'PASS' ? 'The recorded date is within the current 2-year filing period.'
...
function locationCheck(record: CaseRecord): EligibilityCheckInput {
  if (record.respondentLocationStatus === 'SINGAPORE') {
    return check('RESPONDENT_LOCATION', 'PASS', 'The respondent is recorded as located in Singapore.', {
      status: record.respondentLocationStatus,
    });
  }
  if (record.respondentLocationStatus === 'OUTSIDE_SINGAPORE') {
    return check('RESPONDENT_LOCATION', 'FAIL', 'The respondent is recorded as outside Singapore.', {
      status: record.respondentLocationStatus,
    });
  }
  return check('RESPONDENT_LOCATION', 'UNVERIFIED', 'The respondent location has not been established.', {
    status: record.respondentLocationStatus,
  });
}
...
function categoryChecks(record: CaseRecord): EligibilityCheckInput[] {
  if (!record.category) {
    return [check('DISPUTE_CATEGORY', 'UNVERIFIED', 'The dispute category has not been selected.', {
      category: null,
      userConfirmed: record.categoryUserConfirmed,
    })];
  }
  if (!record.categoryUserConfirmed) {
    return [check('DISPUTE_CATEGORY', 'UNVERIFIED', 'The proposed dispute category has not been confirmed by the user.', {
  ...
  if (record.category === 'GENERIC' && record.subtype === 'EMPLOYMENT') {
    return [check('DISPUTE_CATEGORY', 'FAIL', 'Employment matters are outside the Small Claims Tribunals flow.', {
      category: record.category,
  ...
  if (record.category === 'GENERIC') {
    return [check('DISPUTE_CATEGORY', 'UNVERIFIED', 'Generic intake is allowed, but SCT category eligibility still needs authoritative review.', {
      category: record.category,
      userConfirmed: true,
    })];
  }

  const checks = [check('DISPUTE_CATEGORY', 'PASS', 'The confirmed category appears in current Judiciary SCT guidance.', {
    category: record.category,
    userConfirmed: true,
  })];
  ...
  if (record.category === 'PROPERTY_DAMAGE') {
    ...
    if (data?.motorVehicleRelated === true || data?.neighbourCaused === true) {
      checks.push(check('PROPERTY_DAMAGE_EXCLUSION', 'FAIL',
  ...
  if (record.category === 'CPFTA_UNFAIR_PRACTICE') {
    ...
    const complete = data?.relationshipEstablished === true
      && Boolean(data.underlyingTransaction)
      && Boolean(data.exactConduct)
      && data.evidenceIds.length > 0;
    checks.push(check('CPFTA_CONDITIONS', complete ? 'PASS' : 'UNVERIFIED',
      complete
        ...
        : 'The CPFTA intake still needs the relationship, transaction context, exact alleged conduct, or supporting evidence.', {
...
export function assessCase(state: CaseState, today = new Date()): AssessmentResult {
  ...
  const checks = [amountCheck(record), timeCheck(record, today), locationCheck(record), ...categoryChecks(record)];
  const failedChecks = checks.filter((item) => item.result === 'FAIL');
  const unverifiedChecks = checks.filter((item) => item.result === 'UNVERIFIED');
  const eligibilityStatus = failedChecks.length > 0 ? 'FAIL' : unverifiedChecks.length > 0 ? 'UNVERIFIED' : 'PASS';
  const warnings: WarningInput[] = [];

  for (const item of failedChecks) {
    warnings.push(warning(`ELIGIBILITY_${item.code}`, `Known eligibility failure: ${item.explanation}`, item.inputs));
  }
  for (const item of unverifiedChecks) {
    warnings.push(warning(`ELIGIBILITY_${item.code}`, `Eligibility remains unverified: ${item.explanation}`, item.inputs));
  }
  ...
  if (!claimants.some((party) => party.name && party.address)) {
    warnings.push(warning('MISSING_CLAIMANT_PARTICULARS',
      'A claimant name and address have not both been recorded.', claimants.map(({ id, name, address }) => ({ id, name, address }))));
  }
  if (!respondents.some((party) => party.name && party.address)) {
    warnings.push(warning('MISSING_RESPONDENT_PARTICULARS',
      'A respondent name and Singapore service address have not both been recorded.',
      respondents.map(({ id, name, address, country }) => ({ id, name, address, country }))));
  }
  if (!record.factualSummary) {
    warnings.push(warning('MISSING_FACTUAL_SUMMARY', 'The factual summary has not been prepared.', { factualSummary: null }));
  }
  if (state.remedies.length === 0) {
    warnings.push(warning('MISSING_REMEDY', 'No requested remedy has been recorded.', { remedies: [] }));
  }
  if (state.evidence.length === 0) {
    warnings.push(warning('MISSING_EVIDENCE',
      'No evidence is currently uploaded. Ask for relevant alternatives before proceeding.', { evidenceCount: 0 }));
  }

  for (const evidence of state.evidence) {
    if (evidence.processingStatus === 'FAILED' || evidence.processingStatus === 'PARTIAL') {
      warnings.push(warning(`EVIDENCE_${evidence.id}_${evidence.processingStatus}`,
        ...
        { evidenceId: evidence.id, sha256: evidence.sha256, status: evidence.processingStatus, error: evidence.processingError }));
    }
  }

  for (const fact of state.facts.filter((item) => item.material)) {
    if (fact.reviewStatus === 'PENDING') {
      warnings.push(warning(`FACT_${fact.id}_UNREVIEWED`, `Material fact awaiting user review: ${fact.statement}`, {
        factId: fact.id,
        factRevision: fact.revision,
        reviewStatus: fact.reviewStatus,
      }));
    }
    if (['UNASSESSED', 'NOT_FOUND', 'AMBIGUOUS', 'CONTRADICTED'].includes(fact.evidenceAssessment)) {
      warnings.push(warning(`FACT_${fact.id}_EVIDENCE`, `Evidence status for "${fact.statement}": ${fact.evidenceAssessment}.`, {
        factId: fact.id,
        factRevision: fact.revision,
        evidenceAssessment: fact.evidenceAssessment,
      }));
    }
    const looksLikeCashPayment = /\b(cash|paid|payment)\b/i.test(fact.statement)
      && fact.evidenceAssessment !== 'SUPPORTED';
    if (looksLikeCashPayment) {
      warnings.push(warning(`FACT_${fact.id}_CASH_ALTERNATIVES`,
        ...
        { factId: fact.id, factRevision: fact.revision, evidenceAssessment: fact.evidenceAssessment }));
    }
  }

  for (const contradiction of state.contradictions.filter((item) => item.status !== 'RESOLVED')) {
    warnings.push(warning(`CONTRADICTION_${contradiction.id}`,
  ...
  for (const question of state.questions.filter((item) => item.priority === 'REQUIRED' && item.status !== 'ANSWERED')) {
    warnings.push(warning(`QUESTION_${question.id}`,
  ...
  const sensitive = state.extractionRuns.flatMap((run) => run.possibleSensitiveContent.map((item) => ({ runId: run.id, item })));
  if (sensitive.length > 0) {
    warnings.push(warning('PRIVACY_REVIEW_REQUIRED',
      'Potentially unnecessary sensitive content was detected. Review it before filing; originals have not been modified.', sensitive));
  }
  const promptLike = state.extractionRuns.flatMap((run) => run.promptLikeInstructionsObserved.map((item) => ({ runId: run.id, item })));
  if (promptLike.length > 0) {
    warnings.push(warning('EVIDENCE_PROMPT_LIKE_CONTENT',
      'Prompt-like instructions were observed inside evidence and were treated only as source content.', promptLike, false));
  }

  const acknowledged = new Set(state.warnings
  ...
  const reasons = warnings.map((item): ReadinessReason => ({
    ...
    kind: item.code.startsWith('ELIGIBILITY_') ? 'ELIGIBILITY'
      : item.code.startsWith('CONTRADICTION_') ? 'CONTRADICTION'
        : item.code.startsWith('EVIDENCE_') || item.code.includes('_EVIDENCE') ? 'EVIDENCE'
          : item.code.startsWith('PRIVACY_') ? 'PRIVACY'
            : item.code.includes('UNREVIEWED') || item.code.startsWith('QUESTION_') ? 'REVIEW'
              : 'MISSING_INFORMATION',
    acknowledged: acknowledged.has(`${item.code}:${item.fingerprint}`),
    hardBlock: item.code.startsWith('ELIGIBILITY_') && failedChecks.some((failed) => `ELIGIBILITY_${failed.code}` === item.code),
  }));

  const everyWarningAcknowledged = reasons.every((reason) => reason.acknowledged || reason.hardBlock);
  const materialFactsReviewed = state.facts.filter((item) => item.material)
  ...
  } else if (state.contradictions.some((item) => item.status === 'OPEN') && !everyWarningAcknowledged) {
  ...
  const canProceed = eligibilityStatus !== 'FAIL'
...
export function refreshAssessment(store: CaseStore, caseId: string, today = new Date()): AssessmentResult {
