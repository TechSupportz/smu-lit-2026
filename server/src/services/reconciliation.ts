...
function citedExtraction(state: CaseState, item: EvidenceExtractionRecord): string {
  ...
  const location = item.page !== null ? `page ${item.page}` : item.location ?? 'location not recorded';
  return `${evidence?.originalFilename ?? item.evidenceId}, ${location}`;
}

function normalizedName(value: string): string {
  return value.toLowerCase().replace(/\b(pte|private|ltd|limited|llp|company|co)\b/g, '').replace(/[^a-z0-9]/g, '');
}
...
export function reconcileCase(store: CaseStore, caseId: string): void {
  ...
  const documentAmounts = state.extractions
    ...
    .flatMap((item) => {
      const amount = amountFromText(item.value) ?? (item.quote ? amountFromText(item.quote) : null);
      return amount ? [{ item, amount }] : [];
    });

  for (const fact of state.facts.filter((item) => item.sourceType === 'USER_ASSERTION' && item.reviewStatus !== 'REJECTED')) {
    const asserted = factAmount(fact);
    if (!asserted || !/\b(amount|cost|deposit|paid|payment|price|quote|refund|rent|claim)\b/i.test(fact.statement)) continue;
  ...
  const estimatedDates = state.extractions.filter((item) =>
    (item.type === 'DATE' || item.type === 'CONTRACT_TERM')
    && /\b(estimate|estimated|approximately|around|target|indicative)\b/i.test(`${item.value} ${item.quote ?? ''}`));
  for (const fact of state.facts.filter((item) =>
    item.sourceType === 'USER_ASSERTION'
    && item.reviewStatus !== 'REJECTED'
    && /\b(promised|guaranteed|firm|deadline|must|would be completed|would deliver)\b/i.test(item.statement))) {
  ...
  for (const respondent of state.parties.filter((item) => item.role === 'RESPONDENT' && item.kind === 'ENTITY' && item.name)) {
    const expectedName = normalizedName(respondent.name!);
    if (partyExtractions.length === 0 || partyExtractions.some((item) => {
    ...
    const citations = partyExtractions.map((item) => `${item.value} (${citedExtraction(state, item)})`);
