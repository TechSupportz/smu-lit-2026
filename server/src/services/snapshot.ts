...
function safePath(root: string, caseId: string, filename: string): string {
    ...
    if (!candidate.startsWith(`${normalizedRoot}${sep}`))
        throw new ProcessingError("Snapshot path escaped its workspace")
    return candidate
}
...
function buildTimeline(state: CaseState): Array<Record<string, unknown>> {
    return state.facts.flatMap(fact => {
        ...
        const originalWording =
            typeof value?.originalWording === "string" ? value.originalWording : null
...
export interface FinalSnapshot {
    ...
    status: {
        ...
        canProceed: boolean
        handoffPermission:
            "FILING_READY" | "PREPARATION_WITH_WARNINGS" | "BLOCKED_ELIGIBILITY" | "NOT_READY"
    }
...
export class SnapshotService {
    ...
    ): Promise<{ record: SnapshotRecord; snapshot: FinalSnapshot }> {
        return this.store.mutations.run(caseId, async () => {
            ...
            if (!state.case.userReviewed) {
                throw new ProcessingError(
                    "The final case review must be explicitly completed before creating a snapshot.",
                )
            }
            if (state.facts.some(fact => fact.material && fact.reviewStatus === "PENDING")) {
                throw new ProcessingError(
                    "Every material fact must be confirmed, edited, rejected, or marked uncertain before snapshot creation.",
                )
            }
            if (
                ...
                throw new ProcessingError(
                    "Required questions must be answered or explicitly marked unresolved before snapshot creation.",
                )
            }
            ...
            const handoffPermission =
                state.case.eligibilityStatus === "FAIL"
                    ? "BLOCKED_ELIGIBILITY"
                    : state.case.preparationStatus === "READY"
                      ? "FILING_READY"
                      : state.case.preparationStatus === "READY_WITH_WARNINGS"
                        ? "PREPARATION_WITH_WARNINGS"
                        : "NOT_READY"
            const snapshot: FinalSnapshot = {
                ...
                evidenceIndex: state.evidence.map(evidence => ({
                    ...
                    extractionRuns: state.extractionRuns.filter(
                        run => run.evidenceId === evidence.id,
                    ),
                ...
                unresolvedQuestions: state.questions.filter(
                    question => question.status !== "ANSWERED",
                ),
                contradictions: state.contradictions.filter(
                ...
                reviewRecords: {
                    ...
                    factReviews: state.facts.map(({ id: factId, reviewStatus, revision }) => ({
                        factId,
                        reviewStatus,
                        revision,
                    })),
