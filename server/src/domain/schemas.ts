...
export const DisputeModelDataSchema = v.variant("model", [
    v.object({
        model: v.literal("SALE_OF_GOODS"),
        item: OptionalText,
        agreement: OptionalText,
        delivery: OptionalText,
        allegedIssue: OptionalText,
        payments: v.optional(
            v.array(v.object({ amountCents: OptionalCents, wording: OptionalText })),
            [],
        ),
        remedyNotes: OptionalText,
    }),
    v.object({
        model: v.literal("PROVISION_OF_SERVICES"),
        scope: OptionalText,
        quotedTerms: OptionalText,
        promisedTerms: OptionalText,
        performance: OptionalText,
        allegedFailure: OptionalText,
        payments: v.optional(
            v.array(v.object({ amountCents: OptionalCents, wording: OptionalText })),
            [],
        ),
    ...
    v.object({
        model: v.literal("RESIDENTIAL_TENANCY"),
        premises: OptionalText,
        agreementPeriod: OptionalText,
        startDate: OptionalText,
        endDate: OptionalText,
        depositCents: OptionalCents,
        rentCents: OptionalCents,
        disputedObligation: OptionalText,
        remedyNotes: OptionalText,
    }),
    v.object({
        model: v.literal("PROPERTY_DAMAGE"),
        incident: OptionalText,
        ownership: OptionalText,
        allegedActor: OptionalText,
        causationAccount: OptionalText,
        damageEvidence: OptionalText,
        lossCalculation: OptionalText,
        motorVehicleRelated: v.optional(v.nullable(v.boolean()), null),
        neighbourCaused: v.optional(v.nullable(v.boolean()), null),
    }),
    v.object({
        model: v.literal("CPFTA_UNFAIR_PRACTICE"),
        consumerSupplierRelationship: OptionalText,
        relationshipEstablished: v.optional(v.nullable(v.boolean()), null),
        underlyingTransaction: OptionalText,
        exactConduct: OptionalText,
        occurredAt: OptionalText,
        allegedLossCents: OptionalCents,
        remedyNotes: OptionalText,
        evidenceIds: OptionalEvidenceIds,
    }),
    v.object({
        model: v.literal("MOTOR_VEHICLE_DEPOSIT_REFUND"),
        dealer: OptionalText,
        proposedTransaction: OptionalText,
        depositCents: OptionalCents,
        agreement: OptionalText,
        cancellationFacts: OptionalText,
        refundFacts: OptionalText,
        evidenceIds: OptionalEvidenceIds,
    }),
    v.object({
        model: v.literal("GENERIC"),
...
export const RetrievalStatusSchema = v.picklist([
    "RETRIEVED",
    "FAILED",
    "NOT_ATTEMPTED",
    "HISTORICAL",
...
export const ProcessingStatusSchema = v.picklist([
    "PENDING",
    "PROCESSING",
    "PROCESSED",
    "PARTIAL",
...
export const CasePatchFieldsSchema = v.object({
    ...
    causeOfActionDateOriginal: v.optional(
    ...
    officialAssessmentReference: v.optional(
        v.nullable(v.pipe(v.string(), v.trim(), v.maxLength(300))),
    ),
...
export const UpsertPartyInputSchema = v.object({
    ...
    identificationType: v.optional(
        v.nullable(v.pipe(v.string(), v.trim(), v.maxLength(100))),
        null,
    ),
    identificationNumber: v.optional(
        v.nullable(v.pipe(v.string(), v.trim(), v.maxLength(200))),
        null,
    ),
...
export const ExtractionOutputSchema = v.object({
    ...
    pagesInspected: v.pipe(
        v.array(v.pipe(v.number(), v.integer(), v.minValue(1))),
        v.maxLength(500),
    ),
    unreadablePages: v.pipe(
        v.array(v.pipe(v.number(), v.integer(), v.minValue(1))),
        v.maxLength(500),
    ),
    possibleSensitiveContent: v.pipe(
        v.array(v.pipe(v.string(), v.maxLength(500))),
        v.maxLength(100),
    ),
    promptLikeInstructionsObserved: v.pipe(
        v.array(v.pipe(v.string(), v.maxLength(500))),
        v.maxLength(100),
    ),
