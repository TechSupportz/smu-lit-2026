...
const ConfigSchema = v.object({
  ...
  OPENCODE_GO_BASE_URL: v.optional(v.pipe(v.string(), v.url()), 'https://opencode.ai/zen/go/v1'),
  OPENCODE_GO_MODEL: v.optional(v.string(), 'muse-spark-1.3-contributor'),
  OPENROUTER_API_KEY: v.optional(v.string(), ''),
  OPENROUTER_BASE_URL: v.optional(v.pipe(v.string(), v.url()), 'https://openrouter.ai/api/v1'),
  OPENROUTER_EXTRACTION_MODEL: v.optional(v.string(), 'google/gemini-3.8-flash'),
  HOST: v.optional(v.string(), '127.0.0.1'),
  PORT: integerFromString(3000),
  CORS_ORIGINS: v.optional(v.string(), 'http://localhost:5173'),
