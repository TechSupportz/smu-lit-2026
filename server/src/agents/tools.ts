...
function withAssessment<T>(
    caseId: string,
    value: T,
): { value: T; caseRevision: number; assessment: ReturnType<typeof assessCase> } {
...
export function useSctTools(caseId: string): void {
    useTool({
        name: "get_case_state",
        description:
            "Read the complete structured state, provenance, warnings, checks, and current revision for this case.",
        run() {
            const assessment = refreshAssessment(caseStore, caseId)
    ...
    useTool({
        ...
        description:
            "Update case details from the conversation. This cannot confirm the final review or confirm a proposed category for the user.",
        input: v.object({
        ...
        async run({ data }) {
            return caseStore.mutations.run(caseId, () => {
                ...
                const value = caseStore.updateCase(
                    caseId,
                    { expectedRevision: data.expectedRevision, patch },
                    "AGENT",
                )
    ...
    useTool({
        name: "upsert_party",
        description:
            "Create or update claimant/respondent particulars while preserving the case revision check.",
    ...
    useTool({
        name: "upsert_remedy",
        description:
            "Create or update a requested remedy and amount. Record discrepancies instead of silently reconciling them.",
    ...
    useTool({
        name: "inspect_file",
        description:
            "Inspect stored extraction runs, cited items, and fact links for one uploaded evidence file.",
        ...
        run({ data }) {
            ...
            return toolOutput({
                evidence,
                extractionRuns: state.extractionRuns.filter(
                    run => run.evidenceId === data.evidenceId,
                ),
                extractions: state.extractions.filter(item => item.evidenceId === data.evidenceId),
                factLinks: state.factEvidenceLinks.filter(
                    link => link.evidenceId === data.evidenceId,
                ),
            })
        },
    })
    ...
    useTool({
        ...
        description:
            "Run the bounded OpenRouter extraction for one evidence original and preserve partial/unreadable-page status.",
        input: v.object({ evidenceId: ToolIdSchema, expectedRevision: RevisionSchema }),
        async run({ data }) {
            return caseStore.mutations.run(caseId, async () => {
                const value = await evidenceService.extract(
                    caseId,
                    data.evidenceId,
    ...
    useTool({
        name: "find_in_files",
        description:
            "Search existing evidence extraction values, quotes, and source locations. This does not claim the whole original was inspected.",
        input: v.object({ query: v.pipe(v.string(), v.trim(), v.minLength(2), v.maxLength(500)) }),
        run({ data }) {
            const query = data.query.toLowerCase()
            const state = caseStore.getCaseState(caseId)
            const matches = state.extractions
                .filter(item =>
                    [item.value, item.quote, item.location].some(value =>
    ...
    useTool({
        ...
        description:
            "Return explicit links and cautious lexical candidates for one fact. Only explicit reviewed links establish support or contradiction.",
        ...
        run({ data }) {
            ...
            const candidates = state.extractions
                .filter(item => {
            ...
            const result = reviewed.some(link => link.relationship === "CONTRADICTS")
                ? "contradicted"
                : reviewed.some(link => link.relationship === "SUPPORTS")
                  ? "supported"
                  : candidates.length > 0
                    ? "ambiguous"
                    : "not_found"
    ...
    useTool({
        ...
        description:
            "Propose an explicit support, contradiction, context, or unclear link with a cited extraction when available.",
        input: v.object({
            expectedRevision: RevisionSchema,
            factId: ToolIdSchema,
    ...
    useTool({
        ...
        description:
            "Record that a question was answered or explicitly remains uncertain; preserve the supplied answer wording.",
        input: v.object({
            expectedRevision: RevisionSchema,
    ...
    useTool({
        ...
        description:
            "Retrieve current procedural guidance only from the configured Singapore Judiciary and Singapore Statutes Online allowlist.",
        input: v.object({
            expectedRevision: RevisionSchema,
        ...
        async run({ data }) {
            return caseStore.mutations.run(caseId, async () => {
                ...
                return toolOutput({
                    result,
                    persistedRequirements: value,
                    assessment: refreshAssessment(caseStore, caseId),
                })
            })
        },
    })
    ...
    useTool({
        ...
        description:
            "Evaluate eligibility and preparation separately and return structured reasons without turning warnings into failures.",
        run() {
            const assessment = refreshAssessment(caseStore, caseId)
    ...
    useTool({
        ...
        description:
            "Save an immutable revision-bound JSON snapshot after required user review. A failed eligibility check remains a blocked handoff.",
    ...
    useTool({
        ...
        description:
            "Compile one immutable saved snapshot into the bounded Typst case-summary PDF.",
