...
export class CaseStore {
  ...
  createCase(input: CreateCaseInput): CaseRecord {
    ...
    if (input.idempotencyKey) {
      const existing = this.database
        .prepare('SELECT request_hash, response_json FROM idempotency_records WHERE scope = ? AND idempotency_key = ?')
        .get('create-case', input.idempotencyKey) as Row | undefined;
      if (existing) {
        if (existing.request_hash !== requestHash) throw new IdempotencyConflictError('create-case');
    ...
    return this.transaction(() => {
      ...
      `).run(caseId, title, displayName, input.category, Number(input.category !== null), timestamp, timestamp);
      ...
      this.audit(caseId, 'CASE_CREATED', 'USER', { title, claimantName, category: input.category }, 1);
      ...
      if (input.idempotencyKey) {
        this.database.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run('create-case', input.idempotencyKey, requestHash, JSON.stringify(created), timestamp);
      }
      return created;
    });
  }
  ...
  listCases(): CaseRecord[] {
    return (this.database.prepare('SELECT * FROM cases ORDER BY updated_at DESC').all() as Row[]).map(caseFromRow);
  }

  getCase(caseId: string): CaseRecord {
    const row = this.database.prepare('SELECT * FROM cases WHERE id = ?').get(caseId) as Row | undefined;
  ...
  private bumpCaseRevision(caseId: string, expectedRevision: number, resetUserReview = true): number {
    const timestamp = now();
  ...
  updateCase(caseId: string, input: UpdateCaseInput, actor: 'USER' | 'AGENT' = 'USER'): CaseRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const mapping: Record<string, string> = {
        title: 'title', stage: 'stage', category: 'category',
        categoryConfidence: 'category_confidence', categoryUserConfirmed: 'category_user_confirmed',
        modelData: 'model_data_json', subtype: 'subtype',
        factualSummary: 'factual_summary', causeOfActionDate: 'cause_of_action_date',
        causeOfActionDateOriginal: 'cause_of_action_date_original',
        causeOfActionDatePrecision: 'cause_of_action_date_precision',
        claimAmountCents: 'claim_amount_cents', consentStatus: 'consent_status',
        respondentLocationStatus: 'respondent_location_status',
        officialAssessmentCompleted: 'official_assessment_completed',
        officialAssessmentReference: 'official_assessment_reference', userReviewed: 'user_reviewed',
      };
      ...
      for (const [key, value] of Object.entries(input.patch)) {
        ...
        else if (value === null || typeof value === 'string' || typeof value === 'number') values.push(value);
        else throw new Error(`Unsupported case patch field: ${key}`);
      }
      const materialFields = Object.keys(input.patch).filter((field) => !['title', 'stage', 'userReviewed'].includes(field));
      const nextRevision = this.bumpCaseRevision(caseId, input.expectedRevision, materialFields.length > 0);
      if (assignments.length > 0) {
        values.push(now(), caseId);
        this.database.prepare(`UPDATE cases SET ${assignments.join(', ')}, updated_at = ? WHERE id = ?`).run(...values);
      }
      if (input.patch.title !== undefined) {
        const claimant = this.database
          .prepare("SELECT name FROM parties WHERE case_id = ? AND role = 'CLAIMANT' AND is_primary = 1 ORDER BY created_at LIMIT 1")
          .get(caseId) as Row | undefined;
        const displayName = input.patch.title?.trim() || (claimant?.name as string | undefined) || 'Untitled case';
        this.database.prepare('UPDATE cases SET display_name = ? WHERE id = ?').run(displayName, caseId);
      }
      this.audit(caseId, 'CASE_UPDATED', actor, { changedFields: Object.keys(input.patch) }, nextRevision);
      return this.getCase(caseId);
    });
  }

  upsertParty(caseId: string, input: UpsertPartyInput, actor: 'USER' | 'AGENT' = 'USER'): PartyRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const timestamp = now();
      const partyId = input.partyId ?? id('party');
      const existing = input.partyId
        ? this.database.prepare('SELECT * FROM parties WHERE id = ? AND case_id = ?').get(input.partyId, caseId) as Row | undefined
        : undefined;
      ...
      if (existing) {
        ...
        `).run(input.role, input.kind, input.name, input.identificationType, input.identificationNumber,
          input.phone, input.email, input.address, input.country, Number(input.isPrimary), timestamp, partyId, caseId);
      } else {
        this.database.prepare(`
          INSERT INTO parties (
        ...
        `).run(partyId, caseId, input.role, input.kind, input.name, input.identificationType,
          input.identificationNumber, input.phone, input.email, input.address, input.country,
          Number(input.isPrimary), timestamp, timestamp);
      }
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, existing ? 'PARTY_UPDATED' : 'PARTY_CREATED', actor, { partyId, role: input.role }, revision);
      return partyFromRow(this.database.prepare('SELECT * FROM parties WHERE id = ?').get(partyId) as Row);
    });
  }

  proposeFact(caseId: string, input: ProposeFactInput, actor: 'USER' | 'AGENT' = 'AGENT'): FactRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const timestamp = now();
      ...
      `).run(factId, caseId, input.statement,
        input.structuredValue === null ? null : JSON.stringify(input.structuredValue),
        input.sourceType, input.sourceMessageId, Number(input.material), timestamp, timestamp);
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'FACT_PROPOSED', actor, { factId, sourceType: input.sourceType }, revision);
      return factFromRow(this.database.prepare('SELECT * FROM facts WHERE id = ?').get(factId) as Row);
    });
  }
  ...
  reviewFact(caseId: string, factId: string, input: ReviewFactInput): FactRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const row = this.database.prepare('SELECT * FROM facts WHERE id = ? AND case_id = ?').get(factId, caseId) as Row | undefined;
      if (!row) throw new NotFoundError('Fact', factId);
      const timestamp = now();
      const reviewStatus = input.action === 'CONFIRM' ? 'CONFIRMED'
        : input.action === 'REJECT' ? 'REJECTED'
          : input.action === 'MARK_UNCERTAIN' ? 'UNCERTAIN' : 'CONFIRMED';
      const statement = input.action === 'EDIT' ? input.statement : row.statement;
      const structured = input.action === 'EDIT'
        ? (input.structuredValue === null ? null : JSON.stringify(input.structuredValue))
        : row.structured_value_json;
      this.database.prepare(`
        UPDATE facts SET statement = ?, structured_value_json = ?, review_status = ?,
          revision = revision + 1, updated_at = ? WHERE id = ?
      `).run(statement as SQLInputValue, structured as SQLInputValue, reviewStatus, timestamp, factId);
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
  ...
  upsertRemedy(caseId: string, input: UpsertRemedyInput, actor: 'USER' | 'AGENT' = 'USER'): RemedyRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const timestamp = now();
      const remedyId = input.remedyId ?? id('remedy');
      const existing = input.remedyId
        ? this.database.prepare('SELECT * FROM remedies WHERE id = ? AND case_id = ?').get(input.remedyId, caseId) as Row | undefined
        : undefined;
      ...
      if (existing) {
        this.database.prepare(`
          UPDATE remedies SET type = ?, description = ?, amount_cents = ?, basis = ?,
            revision = revision + 1, updated_at = ? WHERE id = ?
        `).run(input.type, input.description, input.amountCents, input.basis, timestamp, remedyId);
      } else {
        this.database.prepare(`
          INSERT INTO remedies(id, case_id, type, description, amount_cents, basis, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(remedyId, caseId, input.type, input.description, input.amountCents, input.basis, timestamp, timestamp);
      }
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, existing ? 'REMEDY_UPDATED' : 'REMEDY_CREATED', actor, { remedyId }, revision);
      return remedyFromRow(this.database.prepare('SELECT * FROM remedies WHERE id = ?').get(remedyId) as Row);
    });
  }

  addEvidenceMetadata(caseId: string, input: {
    ...
    return this.transaction(() => {
      ...
      `).run(
        evidenceId, caseId, input.originalFilename, input.mimeType, input.storageKey,
        input.sha256, input.sizeBytes, input.documentType, input.description,
        JSON.stringify(input.relevantPages), input.pageCount, timestamp, timestamp,
      );
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'EVIDENCE_UPLOADED', 'USER', {
        evidenceId,
        filename: input.originalFilename,
        sha256: input.sha256,
        sizeBytes: input.sizeBytes,
      }, revision);
      return evidenceFromRow(this.database.prepare('SELECT * FROM evidence_documents WHERE id = ?').get(evidenceId) as Row);
    });
  }
  ...
  markEvidenceProcessing(caseId: string, evidenceId: string): EvidenceRecord {
    ...
    this.audit(caseId, 'EVIDENCE_PROCESSING_STARTED', 'SYSTEM', { evidenceId }, this.getCase(caseId).revision);
    return this.getEvidence(caseId, evidenceId);
  }

  saveExtraction(caseId: string, evidenceId: string, input: {
    expectedRevision: number;
    model: string;
    ...
    return this.transaction(() => {
      ...
      `).run(runId, caseId, evidenceId, input.model, input.output.summary, input.output.documentTitle,
        JSON.stringify(input.output.pagesInspected), JSON.stringify(input.output.unreadablePages),
        JSON.stringify(input.output.possibleSensitiveContent),
        JSON.stringify(input.output.promptLikeInstructionsObserved), Number(input.output.inspectionComplete),
        input.rawResponseHash, timestamp);
      ...
      for (const item of input.output.items) {
        insertItem.run(id('item'), caseId, evidenceId, item.type, item.value, item.page ?? null,
          item.quote ?? null, item.location ?? null, item.confidence ?? null, input.model, runId, timestamp);
      }
      const status = input.output.inspectionComplete && input.output.unreadablePages.length === 0 ? 'PROCESSED' : 'PARTIAL';
      this.database.prepare(`
        UPDATE evidence_documents SET processing_status = ?, processing_error = NULL, updated_at = ?
        WHERE id = ? AND case_id = ?
      `).run(status, timestamp, evidenceId, caseId);
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'EVIDENCE_EXTRACTION_SAVED', 'AGENT', {
        evidenceId,
        runId,
        model: input.model,
  ...
  failEvidenceProcessing(caseId: string, evidenceId: string, input: {
    expectedRevision: number;
    error: string;
  }): EvidenceRecord {
    return this.transaction(() => {
      ...
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'EVIDENCE_PROCESSING_FAILED', 'SYSTEM', { evidenceId, error: input.error.slice(0, 500) }, revision);
      return this.getEvidence(caseId, evidenceId);
    });
  }

  getExtractionRun(runId: string): ExtractionRunRecord {
    const row = this.database.prepare('SELECT * FROM extraction_runs WHERE id = ?').get(runId) as Row | undefined;
    if (!row) throw new NotFoundError('Extraction run', runId);
    return {
      id: String(row.id), caseId: String(row.case_id), evidenceId: String(row.evidence_id),
      model: String(row.model), summary: String(row.summary),
      ...
      promptLikeInstructionsObserved: parseJson<string[]>(row.prompt_like_instructions_json, []),
      inspectionComplete: bool(row.inspection_complete), createdAt: String(row.created_at),
    };
  }

  linkFactToEvidence(caseId: string, input: {
    expectedRevision: number;
    factId: string;
    evidenceId: string;
    extractionId?: string;
    ...
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const fact = this.database.prepare('SELECT id FROM facts WHERE id = ? AND case_id = ?').get(input.factId, caseId);
      if (!fact) throw new NotFoundError('Fact', input.factId);
      this.getEvidence(caseId, input.evidenceId);
      if (input.extractionId) {
        const extraction = this.database.prepare(`
      ...
      `).run(linkId, caseId, input.factId, input.evidenceId, input.extractionId ?? null,
        input.relationship, timestamp, timestamp);
      const assessment = input.relationship === 'SUPPORTS' ? 'SUPPORTED'
        : input.relationship === 'CONTRADICTS' ? 'CONTRADICTED' : 'AMBIGUOUS';
      this.database.prepare(`
      ...
      return this.factEvidenceLinkFromRow(
        this.database.prepare('SELECT * FROM fact_evidence_links WHERE id = ?').get(linkId) as Row,
      );
    });
  }

  addQuestion(caseId: string, input: {
    expectedRevision: number;
    question: string;
    reason: string;
    ...
    return this.transaction(() => {
      ...
      `).run(questionId, caseId, input.question, input.reason, JSON.stringify(input.relatedFactIds),
        input.priority, timestamp, timestamp);
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'QUESTION_ADDED', 'AGENT', { questionId, priority: input.priority }, revision);
      return questionFromRow(this.database.prepare('SELECT * FROM questions WHERE id = ?').get(questionId) as Row);
    });
  }

  resolveQuestion(caseId: string, questionId: string, input: {
    expectedRevision: number;
    ...
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const existing = this.database.prepare('SELECT id FROM questions WHERE id = ? AND case_id = ?').get(questionId, caseId);
      ...
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'QUESTION_RESOLVED', 'USER', { questionId, status: input.status }, revision);
      return questionFromRow(this.database.prepare('SELECT * FROM questions WHERE id = ?').get(questionId) as Row);
    });
  }

  addContradiction(caseId: string, input: {
    expectedRevision: number;
    description: string;
    factIds: string[];
    evidenceIds: string[];
    ...
    return this.transaction(() => {
      ...
      `).run(contradictionId, caseId, input.description, JSON.stringify(input.factIds),
        JSON.stringify(input.evidenceIds), input.severity, timestamp, timestamp);
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'CONTRADICTION_ADDED', 'AGENT', { contradictionId, severity: input.severity }, revision);
      return contradictionFromRow(this.database.prepare('SELECT * FROM contradictions WHERE id = ?').get(contradictionId) as Row);
    });
  }

  resolveContradiction(caseId: string, contradictionId: string, input: {
    expectedRevision: number;
    status: 'RESOLVED' | 'ACCEPTED_UNCERTAINTY';
  }): ContradictionRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const existing = this.database.prepare('SELECT id FROM contradictions WHERE id = ? AND case_id = ?')
      ...
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'CONTRADICTION_REVIEWED', 'USER', { contradictionId, status: input.status }, revision);
      return contradictionFromRow(this.database.prepare('SELECT * FROM contradictions WHERE id = ?').get(contradictionId) as Row);
    });
  }

  applyAssessment(caseId: string, input: {
    ...
    return this.transaction(() => {
      ...
      const timestamp = now();
      const activeWarningKeys = new Set(input.warnings.map((warning) => `${warning.code}:${warning.fingerprint}`));
      const warningRows = this.database.prepare('SELECT id, code, fingerprint FROM warnings WHERE case_id = ? AND status != ?')
        .all(caseId, 'STALE') as Row[];
      for (const row of warningRows) {
        if (!activeWarningKeys.has(`${String(row.code)}:${String(row.fingerprint)}`)) {
          this.database.prepare("UPDATE warnings SET status = 'STALE', updated_at = ? WHERE id = ?")
      ...
      for (const warning of input.warnings) {
        upsertWarning.run(id('warning'), caseId, warning.code, warning.message,
          Number(warning.prominent), warning.fingerprint, input.assessedRevision, timestamp, timestamp);
      }
      ...
      for (const check of input.checks) {
        upsertCheck.run(id('check'), caseId, check.code, check.result, check.explanation,
          JSON.stringify(check.inputs), check.sourceUrl, check.sourceVersion, check.checkedAt, check.retrievalStatus);
      }

      this.database.prepare(`
        UPDATE cases SET eligibility_status = ?, preparation_status = ?, can_proceed = ?, updated_at = ?
        WHERE id = ? AND revision = ?
      `).run(input.eligibilityStatus, input.preparationStatus, Number(input.canProceed), timestamp,
        caseId, input.assessedRevision);
      this.audit(caseId, 'ASSESSMENT_REFRESHED', 'SYSTEM', {
  ...
  recordGuidance(caseId: string, input: {
    expectedRevision: number;
    query: string;
    checkedAt: string;
    sources: Array<{ url: string; status: 'RETRIEVED' | 'FAILED'; excerpts: string[]; error?: string }>;
    ...
    return this.transaction(() => {
      ...
      for (const source of input.sources) {
        ...
        const description = source.status === 'RETRIEVED'
          ? `Query: ${input.query}\n${source.excerpts.join('\n\n')}`.slice(0, 20_000)
          : `Query: ${input.query}\nRetrieval failed: ${source.error ?? 'Unknown error'}`;
        upsert.run(id('requirement'), caseId, code,
          source.status === 'RETRIEVED' ? 'CHECKED' : 'UNVERIFIED', description,
          source.url, input.checkedAt.slice(0, 10), source.status, input.checkedAt);
      }
      const revision = this.bumpCaseRevision(caseId, input.expectedRevision);
      this.audit(caseId, 'OFFICIAL_GUIDANCE_RETRIEVED', 'AGENT', {
        query: input.query,
        sources: input.sources.map(({ url, status }) => ({ url, status })),
      }, revision);
      return (this.database.prepare('SELECT * FROM procedural_requirements WHERE case_id = ? ORDER BY code')
        .all(caseId) as Row[]).map((row) => ({
        id: String(row.id), caseId: String(row.case_id), code: String(row.code), status: String(row.status),
        description: String(row.description), sourceUrl: row.source_url as string | null,
        sourceVersion: row.source_version as string | null, retrievalStatus: String(row.retrieval_status),
        checkedAt: String(row.checked_at),
      }));
    });
  }

  acknowledgeWarning(caseId: string, warningId: string, input: {
    expectedRevision: number;
    fingerprint: string;
  }): WarningRecord {
    return this.transaction(() => {
      this.assertRevision(caseId, input.expectedRevision);
      const row = this.database.prepare('SELECT * FROM warnings WHERE id = ? AND case_id = ?')
      ...
      if (warning.fingerprint !== input.fingerprint || warning.status === 'STALE') {
        throw new RevisionConflictError(caseId, input.expectedRevision, this.getCase(caseId).revision);
      }
      ...
      this.audit(caseId, 'WARNING_ACKNOWLEDGED', 'USER', {
        warningId,
        code: warning.code,
        fingerprint: warning.fingerprint,
        decision: 'PROCEED_WITH_WARNING',
      }, revision);
      return warningFromRow(this.database.prepare('SELECT * FROM warnings WHERE id = ?').get(warningId) as Row);
    });
  }
  ...
  createSnapshotRecord(input: Omit<SnapshotRecord, 'supersededBySnapshotId'>): SnapshotRecord {
    return this.transaction(() => {
      ...
      if (input.supersedesSnapshotId) {
        const previous = this.database.prepare('SELECT id FROM snapshots WHERE id = ? AND case_id = ?')
      ...
      `).run(input.id, input.caseId, input.caseRevision, input.displayName, input.basename,
        input.jsonPath, input.jsonSha256, input.pdfPath, input.pdfSha256,
        input.supersedesSnapshotId, input.createdAt);
      if (input.supersedesSnapshotId) {
        this.database.prepare('UPDATE snapshots SET superseded_by_snapshot_id = ? WHERE id = ?')
          .run(input.id, input.supersedesSnapshotId);
      }
      this.audit(input.caseId, 'SNAPSHOT_CREATED', 'USER', {
        snapshotId: input.id,
  ...
  attachSnapshotPdf(caseId: string, snapshotId: string, pdfPath: string, pdfSha256: string): SnapshotRecord {
    return this.transaction(() => {
      const snapshot = this.getSnapshot(caseId, snapshotId);
      if (snapshot.pdfPath) return snapshot;
      this.database.prepare('UPDATE snapshots SET pdf_path = ?, pdf_sha256 = ? WHERE id = ? AND case_id = ?')
        .run(pdfPath, pdfSha256, snapshotId, caseId);
      this.audit(caseId, 'SNAPSHOT_PDF_COMPILED', 'SYSTEM', { snapshotId, pdfSha256 }, snapshot.caseRevision);
      return this.getSnapshot(caseId, snapshotId);
    });
  }

  getSnapshot(caseId: string, snapshotId: string): SnapshotRecord {
    const row = this.database.prepare('SELECT * FROM snapshots WHERE id = ? AND case_id = ?')
      .get(snapshotId, caseId) as Row | undefined;
    if (!row) throw new NotFoundError('Snapshot', snapshotId);
    return snapshotFromRow(row);
  }

  beginTurn(caseId: string, input: { idempotencyKey: string; requestHash: string }): {
    id: string;
    duplicate: boolean;
    ...
    if (existing) {
      if (existing.request_hash !== input.requestHash) throw new IdempotencyConflictError(`turn:${caseId}`);
      return {
    ...
    return { id: turnId, duplicate: false, submissionId: null, agentUid: null, acceptedAt: null, status: 'ADMITTING' };
  }

  finishTurnAdmission(turnId: string, receipt: { submissionId: string; uid: string; acceptedAt: string }): void {
    this.database.prepare(`
  ...
  getCaseState(caseId: string): CaseState {
    ...
    const extractionRuns = all('SELECT * FROM extraction_runs WHERE case_id = ? ORDER BY created_at').map((row) => ({
      id: String(row.id), caseId: String(row.case_id), evidenceId: String(row.evidence_id),
      model: String(row.model), summary: String(row.summary), documentTitle: row.document_title as string | null,
      pagesInspected: parseJson<number[]>(row.pages_inspected_json, []),
      unreadablePages: parseJson<number[]>(row.unreadable_pages_json, []),
      possibleSensitiveContent: parseJson<string[]>(row.possible_sensitive_content_json, []),
      promptLikeInstructionsObserved: parseJson<string[]>(row.prompt_like_instructions_json, []),
    ...
    const extractions = all('SELECT * FROM evidence_extractions WHERE case_id = ? ORDER BY created_at').map((row) => ({
      id: String(row.id), caseId: String(row.case_id), evidenceId: String(row.evidence_id),
      type: String(row.extraction_type), value: String(row.value), page: row.page as number | null,
      quote: row.quote as string | null, location: row.location as string | null,
      confidence: row.confidence as number | null, model: String(row.model),
      extractionRunId: String(row.extraction_run_id), createdAt: String(row.created_at),
    }));
    return {
      case: record,
      parties: all('SELECT * FROM parties WHERE case_id = ? ORDER BY role, is_primary DESC, created_at').map(partyFromRow),
      facts: all('SELECT * FROM facts WHERE case_id = ? ORDER BY created_at').map(factFromRow),
      evidence: all('SELECT * FROM evidence_documents WHERE case_id = ? ORDER BY uploaded_at').map(evidenceFromRow),
      extractionRuns,
      extractions,
      factEvidenceLinks: all('SELECT * FROM fact_evidence_links WHERE case_id = ? ORDER BY created_at')
        .map((row) => this.factEvidenceLinkFromRow(row)),
      questions: all('SELECT * FROM questions WHERE case_id = ? ORDER BY created_at').map(questionFromRow),
      contradictions: all('SELECT * FROM contradictions WHERE case_id = ? ORDER BY created_at').map(contradictionFromRow),
      eligibilityChecks: all('SELECT * FROM eligibility_checks WHERE case_id = ? ORDER BY code').map(eligibilityFromRow),
      remedies: all('SELECT * FROM remedies WHERE case_id = ? ORDER BY created_at').map(remedyFromRow),
      proceduralRequirements: all('SELECT * FROM procedural_requirements WHERE case_id = ? ORDER BY code').map((row) => ({
        id: String(row.id), caseId: String(row.case_id), code: String(row.code), status: String(row.status),
        description: String(row.description), sourceUrl: row.source_url as string | null,
        sourceVersion: row.source_version as string | null, retrievalStatus: String(row.retrieval_status),
        checkedAt: String(row.checked_at),
      })),
      warnings: all("SELECT * FROM warnings WHERE case_id = ? AND status != 'STALE' ORDER BY prominent DESC, created_at")
        .map(warningFromRow),
      snapshots: all('SELECT * FROM snapshots WHERE case_id = ? ORDER BY created_at DESC').map(snapshotFromRow),
    };
  }
  ...
  deleteCaseRecords(caseId: string): { evidencePaths: string[]; snapshotPaths: string[] } {
    return this.transaction(() => {
      this.getCase(caseId);
      const evidencePaths = (this.database.prepare('SELECT storage_key FROM evidence_documents WHERE case_id = ?')
        .all(caseId) as Row[]).map((row) => String(row.storage_key));
      const snapshotRows = this.database.prepare('SELECT json_path, pdf_path FROM snapshots WHERE case_id = ?')
        .all(caseId) as Row[];
      const snapshotPaths = snapshotRows.flatMap((row) => [row.json_path, row.pdf_path]
  ...
  audit(caseId: string | null, eventType: string, actorType: string, payload: unknown, caseRevision: number | null): void {
    this.database.prepare(`
      INSERT INTO audit_events(id, case_id, event_type, actor_type, payload_json, case_revision, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id('audit'), caseId, eventType, actorType, JSON.stringify(payload), caseRevision, now());
  }

  private factEvidenceLinkFromRow(row: Row): FactEvidenceLinkRecord {
    return {
      id: String(row.id), caseId: String(row.case_id), factId: String(row.fact_id),
      evidenceId: String(row.evidence_id), extractionId: row.extraction_id as string | null,
