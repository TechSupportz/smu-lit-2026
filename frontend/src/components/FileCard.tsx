...
export function FileCard({
    ...
    return (
        <div className={`file-card ${compact ? "compact" : ""}`}>
            ...
            <div className="file-info">
                ...
                <span>
                    {file.status === "generating"
                        ...
                        : file.status === "failed"
                          ? (file.error ?? "Could not prepare file")
                          : `${Math.max(1, Math.round(file.size / 1024))} KB · ${file.kind === "generated" ? (file.backendStored ? "Backend PDF" : "Sample PDF") : file.backendStored ? "Stored with this case" : "Saved on this device"}`}
                </span>
            </div>
            {file.status === "generating" ? (
            ...
            {file.kind === "evidence" && !file.backendStored && !compact && (
                <Button
                    variant="ghost"
                    size="icon"
