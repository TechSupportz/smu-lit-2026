...
describe("demoChecks", () => {
    ...
    it("blocks claims above $30,000", () => {
        expect(
            check(demoChecks(answers({ amount: "30000.01", consent: true }), today), "value"),
        ).toMatchObject({
            status: "blocked",
        })
    })
    ...
    it("requires consent above $20,000 and passes with consent", () => {
        ...
        expect(
            check(demoChecks(answers({ amount: "20000.01", consent: true }), today), "value"),
        ).toMatchObject({
            status: "passed",
        })
    })
    ...
    it("accepts an event exactly two years before today but blocks an older event", () => {
        expect(
            check(demoChecks(answers({ eventDate: "2024-09-05" }), today), "time"),
        ).toMatchObject({
            status: "passed",
        })
        expect(
            check(demoChecks(answers({ eventDate: "2024-09-04" }), today), "time"),
        ).toMatchObject({
            status: "blocked",
        })
    })
